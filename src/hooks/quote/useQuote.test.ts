...
jest.mock('../../services/random/randomService', () => ({ fetchRandomQuote: jest.fn() }));
jest.mock('../../services/alternative/alternativeService', () => ({ fetchAlternativeQuote: jest.fn() }));
...
describe('useQuote', () => {
    ...
    it('should fetch an alternative quote on random quote fetch failure', async () => {
        (fetchRandomQuote as jest.Mock).mockRejectedValue(new Error('Fetch failed'));
        const mockAlternativeQuote: Quote = { quote: 'Alternative quote.', author: 'Unknown' };
        (fetchAlternativeQuote as jest.Mock).mockResolvedValue(mockAlternativeQuote);
        const { result } = renderHook(() => useQuote());
    ...
    it('should set a default message on failure of both fetches', async () => {
        (fetchRandomQuote as jest.Mock).mockRejectedValue(new Error('Fetch failed'));
        (fetchAlternativeQuote as jest.Mock).mockRejectedValue(new Error('Alternative fetch failed'));
        const { result } = renderHook(() => useQuote());
        await waitFor(() => {
